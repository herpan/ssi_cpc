...
<?php echo card_open('Daftar Penarikan EDC BNI', 'bg-teal', true) ?>
<div class='row'>

	<div class="col-md-6 col-lg-4">
		<?php echo button_card("Download", "download semua data", 'text-blue', 'btn-info', 'fa fa-download', 'bg-blue', 'btn-create', site_url() . 'Bni_edc_pullout_dashboard/Bni_edc_pullout_dashboard/download/' . str_replace('%20', ' ', $this->uri->segment(4)),) ?>
	</div>
	...
	<small>
		<table id='table-detail' class='table' style='width:150%'>
			<thead>
				...
				<tr>
					<th class="nst">No</th>
					<th class="nst">Report_&_Action</th>
	...
	function refresh_table(value_search) {
		...
		table_detail = $('#table-detail').dataTable({
			...
			columnDefs: [
				...
				<?php //SETTING UNTUK KOLOM 1 (ACTION) 
				?> {
					"searchable": false,
					"orderable": false,
					"targets": 1
			...
			orderCellsTop: true,
			"oSearch": {
				"sSearch": "<?php echo str_replace('%20', ' ', $this->uri->segment(4)) ?>"
			}

...
<?php echo card_open('Form','bg-green',true)?>	
	...
	<input hidden class='data-sending' id='id' value='<?php if(isset($id))echo $id?>'>
					...
					<div class='form-group'>
							...
							<input readonly type='text' class='form-control data-sending input-simple-date' placeholder='<?php echo $title->general->desc_required ?>' id='tanggal_penerimaan' value='<?php if(isset($data)) { echo $data->tanggal_penerimaan; }  else { echo date('Y-m-d'); }?>'>
							</div>
					</div>
					</div>

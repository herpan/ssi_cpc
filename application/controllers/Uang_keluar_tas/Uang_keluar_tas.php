<?php
require APPPATH. '/controllers/Uang_keluar_tas/Uang_keluar_tas_config.php';

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Uang_keluar_tas extends CI_Controller {
   private $log_key,$log_temp,$title;
   function __construct(){
        parent::__construct();
		$this->load->model('Uang_keluar_tas/Uang_keluar_tas_model','tmodel');
		$this->log_key ='log_Uang_keluar_tas';
		$this->title = new Uang_keluar_tas_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Uang_keluar_tas/Uang_keluar_tas/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Uang_keluar_tas/Uang_keluar_tas/create',
			'link_update'			=> site_url().'Uang_keluar_tas/Uang_keluar_tas/update',
			'link_delete'			=> site_url().'Uang_keluar_tas/Uang_keluar_tas/delete_multiple',
			'link_create_multiple'			=> site_url().'Uang_keluar_tas/Uang_keluar_tas/create_multiple',
		);
		
		$this->template->load('Uang_keluar_tas/Uang_keluar_tas_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Uang_keluar_tas/Uang_keluar_tas/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Uang_keluar_tas/Uang_keluar_tas_form',$data);
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Uang_keluar_tas/Uang_keluar_tas/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
	...
	public function  create_multiple(){
		$data = array(
			...
			'link_download_template'	=> site_url().'Uang_keluar_tas/Uang_keluar_tas/download_template/'.$this->_token,
			'link_upload_template'		=> site_url().'Uang_keluar_tas/Uang_keluar_tas/upload_template/'.$this->_token,
			'link_back'					=> $this->agent->referrer(),			
		);

<?php
require APPPATH. '/controllers/Pecahan_emisi/Pecahan_emisi_config.php';

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Pecahan_emisi extends CI_Controller {
   private $log_key,$log_temp,$title;
   function __construct(){
        parent::__construct();
		$this->load->model('Pecahan_emisi/Pecahan_emisi_model','tmodel');
		$this->log_key ='log_Pecahan_emisi';
		$this->title = new Pecahan_emisi_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Pecahan_emisi/Pecahan_emisi/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Pecahan_emisi/Pecahan_emisi/create',
			'link_update'			=> site_url().'Pecahan_emisi/Pecahan_emisi/update',
			'link_delete'			=> site_url().'Pecahan_emisi/Pecahan_emisi/delete_multiple',
			'link_create_multiple'			=> site_url().'Pecahan_emisi/Pecahan_emisi/create_multiple',
		);
		
		$this->template->load('Pecahan_emisi/Pecahan_emisi_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Pecahan_emisi/Pecahan_emisi/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Pecahan_emisi/Pecahan_emisi_form',$data);
	...
	public function create_action(){
		...
		*/	
		...
		$field=array('jenis_uang_id'=>$val['jenis_uang_id'],'pecahan_id'=>$val['pecahan_id'],'emisi_id'=>$val['emisi_id']);
		$exist = $this->tmodel->if_exist('',$field);
		if(!$o->not_exist($exist,'#emisi_id')){
			$o->message = 'Jenis uang dengan pecahan dan emisi yang diinput sudah ada';
			echo $o->result();	
			return;
		}
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Pecahan_emisi/Pecahan_emisi/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);

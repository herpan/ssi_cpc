<?php
require APPPATH. '/controllers/Kategori_kondisi/Kategori_kondisi_config.php';

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Kategori_kondisi extends CI_Controller {
   private $log_key,$log_temp,$title;
   function __construct(){
        parent::__construct();
		$this->load->model('Kategori_kondisi/Kategori_kondisi_model','tmodel');
		$this->log_key ='log_Kategori_kondisi';
		$this->title = new Kategori_kondisi_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Kategori_kondisi/Kategori_kondisi/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Kategori_kondisi/Kategori_kondisi/create',
			'link_update'			=> site_url().'Kategori_kondisi/Kategori_kondisi/update',
			'link_delete'			=> site_url().'Kategori_kondisi/Kategori_kondisi/delete_multiple',
			'link_create_multiple'			=> site_url().'Kategori_kondisi/Kategori_kondisi/create_multiple',
		);
		
		$this->template->load('Kategori_kondisi/Kategori_kondisi_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Kategori_kondisi/Kategori_kondisi/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Kategori_kondisi/Kategori_kondisi_form',$data);
	...
	public function create_action(){
		...
		*/	
		...
		if(!$o->not_empty($val['nama_kategori'],'#nama_kategori')){
			echo $o->result();	
			return;
		}

		//mencegah data double
		$field=array('nama_kategori'=>$val['nama_kategori']);
		$exist = $this->tmodel->if_exist('',$field);
		if(!$o->not_exist($exist,'#nama_kategori')){
			echo $o->result();	
			return;
		}
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Kategori_kondisi/Kategori_kondisi/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
	...
	public function  create_multiple(){
		$data = array(
			...
			'link_download_template'	=> site_url().'Kategori_kondisi/Kategori_kondisi/download_template/'.$this->_token,
			'link_upload_template'		=> site_url().'Kategori_kondisi/Kategori_kondisi/upload_template/'.$this->_token,
			'link_back'					=> $this->agent->referrer(),			
		);

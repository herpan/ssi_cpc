<?php
require APPPATH. '/controllers/Uang_selisih/Uang_selisih_config.php';
...
class Uang_selisih extends CI_Controller {
   ...
   function __construct(){
		...
		$this->title = new Uang_selisih_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Uang_selisih/Uang_selisih/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Uang_selisih/Uang_selisih/create',
			'link_update'			=> site_url().'Uang_selisih/Uang_selisih/update',
			'link_delete'			=> site_url().'Uang_selisih/Uang_selisih/delete_multiple',
			'link_create_multiple'			=> site_url().'Uang_selisih/Uang_selisih/create_multiple',
		);
		
		$this->template->load('Uang_selisih/Uang_selisih_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Uang_selisih/Uang_selisih/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Uang_selisih/Uang_selisih_form',$data);
	...
	public function create_action(){
		...
		*/	
		...
		if(!$o->not_exist($exist,'#uang_masuk_id')){
			echo $o->result();	
			return;
		}
		...
		//mencegah data kosong
		if(!$o->not_empty($val['nama_kasir_bank_client'],'#nama_kasir_bank_client')){
			echo $o->result();	
			return;
		}
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Uang_selisih/Uang_selisih/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
	...
	public function  create_multiple(){
		$data = array(
			...
			'link_download_template'	=> site_url().'Uang_selisih/Uang_selisih/download_template/'.$this->_token,
			'link_upload_template'		=> site_url().'Uang_selisih/Uang_selisih/upload_template/'.$this->_token,
			'link_back'					=> $this->agent->referrer(),			
		);

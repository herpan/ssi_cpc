<?php
require APPPATH. '/controllers/Journal_cpc/Journal_cpc_config.php';

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Journal_cpc extends CI_Controller {
   private $log_key,$log_temp,$title;
   function __construct(){
        parent::__construct();
		$this->load->model('Journal_cpc/Journal_cpc_model','tmodel');
		$this->log_key ='log_Journal_cpc';
		$this->title = new Journal_cpc_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Journal_cpc/Journal_cpc/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Journal_cpc/Journal_cpc/create',
			'link_update'			=> site_url().'Journal_cpc/Journal_cpc/update',
			'link_delete'			=> site_url().'Journal_cpc/Journal_cpc/delete_multiple',
			'link_create_multiple'			=> site_url().'Journal_cpc/Journal_cpc/create_multiple',
		);
		
		$this->template->load('Journal_cpc/Journal_cpc_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Journal_cpc/Journal_cpc/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Journal_cpc/Journal_cpc_form',$data);
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Journal_cpc/Journal_cpc/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
	...
	public function  create_multiple(){
		$data = array(
			...
			'link_download_template'	=> site_url().'Journal_cpc/Journal_cpc/download_template/'.$this->_token,
			'link_upload_template'		=> site_url().'Journal_cpc/Journal_cpc/upload_template/'.$this->_token,
			'link_back'					=> $this->agent->referrer(),			
		);
